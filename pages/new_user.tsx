							...
							if (data.errors) {
								data.errors.forEach((err: any) => {
									if (!!err.path) {
										...
										if (err.path === 'password') setConfirmPasswordError(err.message)
									}
		...
		<>
			...
			{!authState && (
				<Container>
					...
					<form autoComplete='off' onSubmit={(e: FormEvent<HTMLFormElement>) => handleSubmit(e)}>
						<Box margin='auto' maxWidth='25rem' display='flex' flexDirection='column'>
							<TextField
								margin='normal'
								color='primary'
								variant='outlined'
